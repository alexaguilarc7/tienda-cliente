...
class AdminProducts extends AdminTab
{
	...
	public function display($token = NULL)
	{
        ...
        $this->getList(intval($cookie->id_lang_2), !$cookie->__get($this->table.'Orderby') ? 'position' : NULL, !$cookie->__get($this->table.'Orderway') ? 'ASC' : NULL);
	...
	public function displayForm($isMainTab = true)
	{
		...
		<form action="'.$currentIndex.'&token='.Tools::getValue('token').'" method="post" enctype="multipart/form-data" name="product" id="product" style="position:relative;">
        ...
        //$this->displayFormTaxes();
		$this->displayFormAttachments($obj, $this->token, $languages, $defaultLanguage);
        $this->displayFormAccesories($obj, $this->token);
        
		//if ($obj->id && 0)
		/*if ($obj->id)
            echo '<div class="tab-page" id="step7""><h4 class="tab">3. '.$this->l('Attachments').'</h4></div>
            <div class="tab-page" id="step8""><h4 class="tab">4. '.$this->l('Accesories').'</h4></div>';
        else
            echo '<div class="tab-page" id="step7""><h4 class="tab">3. '.$this->l('Attachments').'</h4></div>
            <div class="tab-page" id="step8""><h4 class="tab">4. '.$this->l('Accesories').'</h4></div>';*/
			//echo '<div class="tab-page" id="step3"><h4 class="tab">3. '.$this->l('Combinations').'</h4></div>';
			//echo '<div class="tab-page" id="step4"><h4 class="tab">4. '.$this->l('Features').'</h4></div>'; 
			//echo '<div class="tab-page" id="step5"><h4 class="tab">5. '.$this->l('Customization').'</h4></div>';
			//echo '<div class="tab-page" id="step6""><h4 class="tab">6. '.$this->l('Discounts').'</h4></div>';
			//echo '<div class="tab-page" id="step7""><h4 class="tab">3. '.$this->l('Attachments').'</h4></div>';
            //echo '<div class="tab-page" id="step8""><h4 class="tab">4. '.$this->l('Accesories').'</h4></div>';
        ...
        /*echo '
            <script type="text/javascript">
                setTimeout(function(){
                    $("#tabPane1 .tab-row").append("<a id=tax>'.$this->l('4. Tax').'</a>");
                    $("#tax").attr("target","_blank");
                    $("#tax").attr("href", "index.php?tab=AdminTaxes&token='.Tools::getAdminToken("AdminTaxes".intval(Tab::getIdFromClassName("AdminTaxes")).intval($cookie->id_employee)).'");
                }, 2000);
    		</script>
        ';*/
	...
	public function displayFormAttachments($obj, $languages, $defaultLanguage)
	{
        ...
        $attach1 = Attachment::getAttachments($cookie->id_lang_2, $obj->id, true);
		$attach2 = Attachment::getAttachments($cookie->id_lang_2, $obj->id, false);
        ...
        print_r($obj);
        echo "</pre>";*/
		/*echo '<div class="tab-page" id="step1">';
		echo '
        ...
        foreach($langs as $lang):
            if($lang['id_lang'] == $cookie->id_lang_2)
                echo '<td  style="position: relative;"><div style="margin: 0 0 0 10px; position: absolute; top: 26px;"><img src="'._PS_IMG_.'l/'.$lang['id_lang'].'.jpg" alt="'.strtoupper($lang['iso_code']).'" title="'.$lang['name'].'" /></div></td>';
        endforeach;
		echo '<td style="padding-left:20px;">
				<!--<select multiple id="selectAttachment2" style="width:300px;height:160px; margin: 0 0 0 0">';
		...
		echo '	</select>--><br /><br />
                ...
                <a style="display: block; padding: 12px;"></a>
			</div>
			</td>
		</tr></table>
		<div class="clear">&nbsp;</div>
		<input type="submit" name="submitAttachments" id="submitAttachments" value="'.$this->l('Update').'" class="button" style="width: 300px;" />';
		echo '</div>';*/
        ...
        if($id_product != null)
        {
            echo '<script type="text/javascript">
                jQuery(function(){
                    ...
                    var msgerror = \''.$this->l('You must select to remove.').'\';
                    jQuery("#deleteAttachment").click(function(){
                        var attach = jQuery("#selectAttachment1");
                        var id_attachment_lang = attach.attr("value");
                        var attachment = attach.attr("value").split("_");
                        var messageAttachment = msgAttachment+" "+attachment[0];
    
                        if(attach.attr("value") != ""){
                            if(confirm(messageAttachment)){
                                $.post("ajax.php", {id_attachment_lang: id_attachment_lang}, function(){
                                    attach.find("option[value="+attach.attr("value")+"]").remove();    
                                });  
                            }    
                        }else{
                            alert(msgerror);
                    ...
                    //var btnUpload = $(".lang_attachment").attr("isolang");
                    new AjaxUpload("#btn_upload_attachment",{
                        action: "ajax.php?upload=uploadfile&id="+product_id,
                        name: "file",
                        ...
                        onComplete: function(file, response){                     
                            this.enable();
                            var arraydatas = response.split("_");
                            var id_value = arraydatas[0]+"_"+arraydatas[1];
                            var option = "<option value="+id_value+">"+arraydatas[2]+"</option>";
                            //$.fancybox.hideActivity();
    
                            $("#selectAttachment1").append(option);
                        }  
                    });
                }); 
            </script>';   
        ...
        echo '<div class="tab-page" id="step1" '.($id_product != null ? "" : "style='height:180px;'").'>
            <h4 class="tab">3. '.$this->l('Attachments').'</h4>';
        if($id_product != null)
        {
        echo '
            <!--<a href="index.php?tab=AdminAttachments&addattachment&token='.Tools::getAdminToken('AdminAttachments'.intval(Tab::getIdFromClassName('AdminAttachments')).intval($cookie->id_employee)).'">
        ...
        foreach ($attach1 as $attach)
            if($attach['id_attachment'] != "" || $attach['id_attachment'] != 0)
 		         echo '	<option value="'.$attach['id_attachment']."_".$attach['id_lang'].'">'.$attach['name'].'</option>';
		echo '	</select><br /><br />';
        /*if(count($attach1) > 0){
		echo '<a href="#"  id="deleteAttachment" style="text-align:center;display:block;border:1px solid #aaa;text-decoration:none;background-color:#fafafa;color:#123456;margin:2px;padding:2px">
					'.$this->l('Remove').' &gt;&gt;
				</a>';
        }*/
		echo '</td>';
        ...
        foreach($langs as $lang):
            if($lang['id_lang'] == $cookie->id_lang_2)
                echo '
                <div id="lang_attachment_'.$lang['id_lang'].'" class="attachment_translate lang_attachment" isolang="'.$lang['iso_code'].'" style="display:block;"><img onclick="showLangAttachment(this)" src="'._PS_IMG_.'l/'.$lang['id_lang'].'.jpg" alt="'.strtoupper($lang['iso_code']).'" title="'.$lang['name'].'" /></div>
                ';
            else
                echo '
                <div id="lang_attachment_'.$lang['id_lang'].'" class="attachment_translate" isolang="'.$lang['iso_code'].'" style="display:none;"><img onclick="showLangAttachment(this)" src="'._PS_IMG_.'l/'.$lang['id_lang'].'.jpg" alt="'.strtoupper($lang['iso_code']).'" title="'.$lang['name'].'" /></div>
                ';
        endforeach;
        echo '</td><td><div id="lang_tralate_attachment" style="display: none;">Choose language:<br/><br/>';
        foreach($langs as $lang):
            echo '<img id="'.$lang['id_lang'].'" onclick="changeLangAttachment(this, '.$obj->id.')" class="pointer" style="margin: 0px 2px;" src="'._PS_IMG_.'l/'.$lang['id_lang'].'.jpg" alt="'.strtoupper($lang['iso_code']).'" title="'.$lang['name'].'" /></a>';
        endforeach;
        echo '</div></td>';
		...
		<!--<input type="submit" name="submitAttachments" id="submitAttachments" value="'.$this->l('Update').'" class="button" style="width: 300px;" />-->
         <div>
            <div style="text-align: right;width: 302px;">
            <button href="#" type="button" id="deleteAttachment" style="text-align:center;border:1px solid #aaa;text-decoration:none;background-color:#fafafa;color:#123456;margin:2px;padding:3px;width:100px;">
					'.$this->l('Remove').'
		      </button>
            <button type="button" class="button" id="btn_upload_attachment" style="width:100px;">'.$this->l('Upload File').'</button>
            </div>
         </div>
        ';
        }else{
            echo 'Debe crear el producto para subir adjuntos.';
        }
        echo '</div>';
	}
	...
	function displayFormInformations($obj, $currency)
	{
        ...
        $qty_missing = $this->getFieldValue($obj, 'quantity_missing');
        
        //$obj_encode = json_encode($obj);
        //$array_encode = $obj;
        $array_encode = serialize($input_array);
        //$array_encode = urlencode($array_encode);
        //$cookie->category_object = $array_encode;
        
        //$obj_encode = json_encode($input_array);
        
		//includeDatepicker(array('reduction_from', 'reduction_to'));
        
        //echo "<input type='hidden' id='object_id' name ='object_id' value='".$obj_encode."'/>";
		...
		<a href="'.($link->getProductLink($this->getFieldValue($obj, 'id'), $this->getFieldValue($obj, 'link_rewrite', $this->_defaultFormLanguage), Category::getLinkRewrite($this->getFieldValue($obj, 'id_category_default'), intval($cookie->id_lang)))).'"><img src="../img/admin/details.gif" alt="'.$this->l('View product in shop').'" title="'.$this->l('View product in shop').'" /> '.$this->l('View product in shop').'</a>';
			if (0 && file_exists(_PS_MODULE_DIR_.'statsproduct/statsproduct.php'))
				echo '&nbsp;-&nbsp;
		...
		echo '		</td>
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							<select onChange="javascript:calcPriceTI();" name="id_tax" id="id_tax" '.(Tax::excludeTaxeOption() ? 'disabled="disabled"' : '' ).'>
								<option value="0"'.(($this->getFieldValue($obj, 'id_tax') == 0 && !isset($obj->id)) ? ' selected="selected"' : '').'>'.$this->l('No tax').'</option>';
							foreach ($taxes AS $k => $tax)
								echo '
								<option value="'.$tax['id_tax'].'"'.(($this->getFieldValue($obj, 'id_tax') == $tax['id_tax'] || (!isset($obj->id) && $tax['id_tax']==1)) ? ' selected="selected"' : '').'>'.stripslashes($tax['name']).' ('.$tax['rate'].'%)</option>';
							echo '
							</select>';
				...
				echo '</td>
					...
					<tr>
						...
						<td>'.$this->l('available from').' <input type="text" id="reduction_from" name="reduction_from" value="'.(($from = $this->getFieldValue($obj, 'reduction_from') AND $from != '0000-00-00 00:00:00' AND $from != '1942-01-01 00:00:00') ? $from : date('Y-m-d H:i:s')).'" />
							'.$this->l('to').' <input type="text" id="reduction_to" name="reduction_to" value="'.(($to = $this->getFieldValue($obj, 'reduction_to') AND $to != '0000-00-00 00:00:00' AND $to != '1942-01-01 00:00:00') ? $to : date('Y-m-d H:i:s')).'" />
							<p>'.$this->l('Leave same dates for undefined duration').'</p>
						</td>
					</tr>
                    <tr>
						<td class="col-left">'.$this->l('On sale:').'</td>
						<td style="padding-bottom:5px;">
                    ...
                    <tr>
						<td class="col-left"><b>'.$this->l('Sale price:').'</b></td>
						<td style="padding-bottom:5px;">
                ...
                if($obj->id != null){
                            ...
                            <td colspan="2">
                                <div class="row_quantity">
                                    <div class="col_quantity stockname" '.($stockcontrol == 1 ? '' : 'style="border: none;"').'>
                                        ...
                                        <div class="row">
                                            <label class="stock_name">'.$this->l('Negativo:').'</label>
                                            <input size="3" maxlength="6" name="quantity_missing" id="quantity_missing" type="text" value="'.$qty_missing.'" disabled="disabled"/>
                                        </div>
                                    </div>
                                    ...
                                    <div '.($stockcontrol == 1 ? ' class="col_quantity"' : 'class="col_quantity hide_tag"').' style="border-right:1px solid #DDDDDD;margin:0 5px 0 0;padding: 0 0px 0 0;width:230px;">
                                        ...
                                        <div class="row">
                                            ...
                                            <input size="3" maxlength="6" name="quantity_add" id="quantity_add" type="text" value="" class="quantity_add_update"/>
                                            <button type="button" class="button" type_field="add_stock">'.$this->l('Add').'</button>
                                        </div>
                                        <div class="row">
                                            <label class="stock_name_2">'.$this->l('Negativo').':</label>
                                            <input size="3" maxlength="6" name="quantity_add_missing" id="quantity_add_missing" type="text" value="" class="quantity_add_update"/>
                                            <button type="button" class="button" type_field="add_stock_missing">'.$this->l('Rebajar').'</button>
                                        </div>
                                    ...
                                    <div '.($stockcontrol == 1 ? 'class="col_quantity"' : 'class="col_quantity hide_tag"').' style="border-right:1px solid #DDDDDD; margin-right: 10px;">
                                        ...
                                        <div class="row">
                                            <label class="stock_name_2">'.$this->l('Positivo:').'</label>
                                            <input size="3" maxlength="6" name="quantity_update" id="quantity_update" type="text" value="" class="quantity_add_update"/>
                                            <button type="button" class="button" type_field="update_stock">'.$this->l('Restablecer').'</button>
                                        </div>
                                        <div class="row">
                                            <label class="stock_name_2">'.$this->l('Negativo:').'</label>
                                            <input size="3" maxlength="6" name="quantity_update_missing" id="quantity_update_missing" type="text" value="" class="quantity_add_update"/>
                    ...
                    echo '
                    <tr>
                        <td class="col-left">'.$this->l('Stock:').'</td>
						<td style="padding-bottom:5px;">
							<input size="3" maxlength="6" '.$qty_state.' name="quantity" id="quantity" type="text" value="'.$qty.'"/>
						</td>
					</tr>';
                }    
                echo '<tr>
		...
		echo '			</td>
        		...
        		echo '
                                ...
                                <li>
                                    ...
                                    <select name="id_supplier" id="id_supplier" style="margin:0 0 0 30px;">
        								<option value="0">-- '.$this->l('Choose (optional)').' --</option>';
        		...
        		echo '
                                    ...
                                    <div class="box_supp_ref">
                                        <div>
                                            <label>'.$this->l('Supplier Reference:').'</label>
                                        ...
                                        <div>
                                            <label>'.$this->l('Location:').'</label><input size="55" type="text" name="location" value="'.$this->getFieldValue($obj, 'location').'" style="width: 101px;" />
                							<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#\<span class="hint-pointer">&nbsp;</span></span>
                                        </div>
                                    </div>
                                </li>
		...
		echo '			</select>
					...
					</tr>						<tr ><td colspan="2" style="padding-bottom:5px;"></td></tr>
					<tr >
		...
		echo '			</table>
							...
							</div><span id="extra22"></span><span id="extra23"></span>
						</td>
					</tr>
		  ...
		  echo '	<!--<tr>
					...
					<tr>
                        <td>
                            <input id="mostrarop" type="button" onclick="mostrar(1)" value="'.$this->l('More options').'">
                            <input id="ocultarop" style="display:none" type="button" onclick="mostrar(0)" value="Ocultar opciones">
                        </td>
                    </tr>-->
					<tr><td colspan="2"><hr style="width:100%;" /></td></tr>
                    
					<!--<tr id="extra1" style="display:none">
						
					</tr>
					<tr id="extra2" style="display:none">
						
					</tr>
					<tr id="extra3" style="display:none">
						<td class="col-left"></td>
						<td style="padding-bottom:5px;"></td>
					</tr>-->
                    <tr id="extra4" style="display:none">
					...
					<tr id="extra5" style="display:none">
						<!--<td class="col-left">'.$this->l('Weight:').'</td>
						<td style="padding-bottom:5px;">
							<input size="6" maxlength="6" name="weight" type="text" value="'.htmlentities($this->getFieldValue($obj, 'weight'), ENT_COMPAT, 'UTF-8').'" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');" /> '.Configuration::get('PS_WEIGHT_UNIT').'
						</td>-->
					</tr>
						<tr id="extra6" style="display:none"><td colspan="2"><hr style="display:none;width:100%;" /></td></tr>';
					$this->displayPack($obj);
		echo '			<tr id="extra8" style="display:none"><td colspan="2"><hr style="display:none;width:100%;" /></td></tr>';
...
?>
	...
	function mostrar(valor)
	{
    	...
    	{
        	/*if (valor==1)
        	document.getElementById("extra"+i).style.display="";
        	else
        	document.getElementById("extra"+i).style.display="none";*/
            if (valor==1){
                    //document.getElementById("extra"+i).style.display="";
                    jQuery("table").find("#extra"+i).css("display", "");
           	}else{
                //document.getElementById("extra"+i).style.display="none";
                jQuery("table").find("#extra"+i).css("display", "none");   
           	}
    	}

		if (valor==1)
		{
		document.getElementById('mostrarop').style.display="none";
		document.getElementById('ocultarop').style.display="";
		}
		else
		{
		document.getElementById('ocultarop').style.display="none";
		document.getElementById('mostrarop').style.display="";
		}
	...
	?>
		<tr id="extra9" style="display:none">
		<td colspan="2">
			<input style="display:none" type="checkbox" id="is_virtual_good" name="is_virtual_good" value="true" onchange="toggleVirtualProduct(this)" onclick="toggleVirtualProduct(this);" <?php if(($productDownload->id OR Tools::getValue('is_virtual_good')=='true') AND $productDownload->active) echo 'checked="checked"' ?> />
...
: '' ) : htmlentities(Tools::getValue('virtual_product_expiration_date'), ENT_COMPAT, 'UTF-8') ?>" size="11" maxlength="10" autocomplete="off" /> <?php echo $this->l('Format: YYYY-MM-DD'); ?>
	...
	<!--<tr  id="extra10" style="display:none"><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>-->
	<script type="text/javascript">
...
<?php
					...
					<!--<tr id="extra10" style="display:none">
					...
					<!--<tr id="extra11" style="display:none">
						<td class="col-left">'.$this->l('Retail price with tax:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? ' '.$currency->sign : '').' <input size="11" maxlength="14" id="priceTI" type="text" value="" onchange="noComma(\'priceTI\');" onkeyup="calcPriceTE();" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:10px">
					...
					<!--<tr  id="extra12" style="display:none">
						<td class="col-left">'.$this->l('Eco-tax:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? $currency->sign.' ' : '').'<input size="11" maxlength="14" id="ecotax" name="ecotax" type="text" value="'.$this->getFieldValue($obj, 'ecotax').'" onkeyup="this.value = this.value.replace(/,/g, \'.\'); if (parseInt(this.value) > getE(\'priceTE\').value) this.value = getE(\'priceTE\').value; if (isNaN(this.value)) this.value = 0;" />'.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="margin-left:10px">('.$this->l('already included in price').')</span>
					...
					<!--<tr  id="extra13" style="display:none">
						<td class="col-left">'.$this->l('Reduction amount:').'</td>
						<td style="padding-bottom:5px;">
							'.($currency->format == 1 ? ' '.$currency->sign.' ' : '').'<input size="11" maxlength="14" type="text" name="reduction_price" id="reduction_price" value="'.$this->getFieldValue($obj, 'reduction_price').'" onkeyup="javascript:this.value = this.value.replace(/,/g, \'.\'); var key = window.event ? window.event.keyCode : event.which; if (key != 9) reductionPrice();" /> '.($currency->format == 2 ? ' '.$currency->sign : '').'
							<span style="padding-right: 15px; padding-left: 15px; font-weight: bold">'.$this->l('OR').'</span>
					...
					<!--<tr id="extra14" style="display:none">
						<td class="col-left">&nbsp;</td>
						<td>'.$this->l('available from').' <input type="text" id="reduction_from" name="reduction_from" value="'.(($from = $this->getFieldValue($obj, 'reduction_from') AND $from != '0000-00-00 00:00:00' AND $from != '1942-01-01 00:00:00') ? $from : date('Y-m-d H:i:s')).'" />
							'.$this->l('to').' <input type="text" id="reduction_to" name="reduction_to" value="'.(($to = $this->getFieldValue($obj, 'reduction_to') AND $to != '0000-00-00 00:00:00' AND $to != '1942-01-01 00:00:00') ? $to : date('Y-m-d H:i:s')).'" />
							<p>'.$this->l('Leave same dates for undefined duration').'</p>
					...
					<!--<tr  id="extra15" style="display:none">
						<td class="col-left">&nbsp;</td>
						<td style="padding-bottom:5px;">
							<input type="checkbox" name="on_sale" id="on_sale" style="padding-top: 5px;" '.($this->getFieldValue($obj, 'on_sale') ? 'checked="checked"' : '').'value="1" />&nbsp;<label for="on_sale" class="t">'.$this->l('Display "on sale" icon on product page and text on product listing').'</label>
						</td>
					...
					<!--<tr  id="extra16" style="display:none"><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>-->
					<!--<tr  id="extra17" style="display:none">
					...
					<!--<tr  id="extra18" style="display:none">
						<td class="col-left">'.$this->l('Displayed text when in-stock:').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
		...
		echo '			</td>
					...
					<!--<tr  id="extra19" style="display:none">
						<td class="col-left">'.$this->l('Displayed text when allowed to be back-ordered:').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
		foreach ($this->_languages as $language)
			echo '		<div  class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
		...
		echo '			</td>
					...
					<!--<tr id="extra20" style="display:none">
						<td class="col-left">'.$this->l('When out of stock:').'</td>
						<td style="padding-bottom:5px;">
							<input type="radio" name="out_of_stock" id="out_of_stock_1" value="0" '.(intval($this->getFieldValue($obj, 'out_of_stock')) == 0 ? 'checked="checked"' : '').'/> <label for="out_of_stock_1" class="t" id="label_out_of_stock_1">'.$this->l('Deny orders').'</label>
							<br /><input type="radio" name="out_of_stock" id="out_of_stock_2" value="1" '.($this->getFieldValue($obj, 'out_of_stock') == 1 ? 'checked="checked"' : '').'/> <label for="out_of_stock_2" class="t" id="label_out_of_stock_2">'.$this->l('Allow orders').'</label>
					...
					<!--<tr  id="extra21" style="display:none"><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>-->

					<!--<tr id="extra24" style="display:none"><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>-->
					<tr id="extra25" style=""><td colspan="2">
		...
		echo '						<p class="clear">'.$this->l('Keywords for HTML header, separated by a comma').'</p>
								...
								<tr id="extra28" style="">
									<td class="col-left">'.$this->l('Friendly URL:').'<span id="extra29"></span><span id="extra30"></span></td>
									<td class="translatable">';
		foreach ($this->_languages as $language)
			echo '	<div  class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
		...
		echo '						<p class="clear" style="width: 360px; word-wrap: break-word; overflow: auto;">'.$this->l('Product link will look like this:').' '.(Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').$_SERVER['SERVER_NAME'].'/id_product-<span id="friendly-url"></span>.html<br/>
                                        '.$this->l('Automatic optimization for search').'
                                    </p>
									</td>
								</tr>';
		echo '</td></tr></table>
					...
					<tr id="extra31" style="display:none">
						<td class="col-left"></td>
						<!--<td style="padding-bottom:5px;" class="translatable">-->
		...
		echo '		</td>
				...
				echo '
					<tr id="extra32" style="display:none">
				...
				if ($postAccessories = Tools::getValue('inputAccessories'))
				{
					...
					echo '
					<tr id="extra33" style="display:none">
					...
					<tr id="extra34" style=""><td colspan="2" style="padding-bottom:10px;"><hr style="width:100%;" /></td></tr>
					<tr  id="extra35" style="">
						<td colspan="2" style="text-align:center;">
							<input type="submit" value="    '.$this->l('Save').'    " name="submitAdd'.$this->table.'" class="button action_submit_form" />
            ...
            <script type="text/javascript">
                ...
                jQuery(function(){
                    ...
                    var id_product = "'.$obj->id.'";
                    
                    $(".quantity_add_update").keypress(function(evento){
                        var keyPressed = (evento.which) ? evento.which : event.keyCode
                        return !(keyPressed > 31 && (keyPressed < 48 || keyPressed > 57));
                    });
                    if(id_product != ""){
                        jQuery(".row_quantity .col_quantity button").click(function(){
                           var ele = jQuery(this);
                           var cant = ele.parent().find(".quantity_add_update").val();
                           var type = ele.attr("type_field");
                           
                           jQuery.post("ajax.php", {"id_product": id_product, "cant": cant, "type": type}, function(res){
                                var obj = $.parseJSON(res);
                                ele.parent().find(".quantity_add_update").val("");
                                jQuery("#quantity").val(obj.positivo);
                                jQuery("#quantity_missing").val(obj.negativo);
    ...
    public function displayFormTaxes($isMainTab = true)
    {
        global $currentIndex, $cookie;
		
		
		echo '<div class="tab-page" id="step2">
        <h4 class="tab">2. '.$this->l('Images').'</h4>
        dfhjkhgkjdfhgjkdfhgdj
        </div>';
    }
	...
	private function displayPack(Product $obj)
	{
		...
		echo '
		<tr  id="extra7" style="display:none">
			<td>
				<input style="display:none" type="checkbox" name="ppack" id="ppack" value="1"'.($boolPack ? ' checked="checked"' : '').' onclick="openCloseLayer(\'ppackdiv\');" />
				<label class="t" for="ppack" style="display:none">'.$this->l('Pack').'</label>
			</td>
			<td>
    ...
    public function displayFormAccesories($obj, $token = NULL)
    {
        global $currentIndex, $cookie;
		$obj = $this->loadObject(true);
		$languages = Language::getLanguages();
        
        $accessories2 = Accessory::getAccessoriesProduct($cookie->id_lang, $obj->id);
        
        /*echo '<div class="tab-page" id="step8">
            <div id="divAccessories">';
                foreach ($accessories2 as $accessory)
                    echo '<div id="accessory_'.$obj->id.'_'.$accessory['id_product'].'" style="margin:5px 0;">'.$accessory['name'].(!empty($accessory['reference']) ? ' ('.$accessory['reference'].')' : '').'<span onclick="eliminarAccesorio('.$obj->id.', '.$accessory['id_product'].');" style="cursor: pointer;"><img src="../img/admin/delete.gif" class="middle" alt="" /></span></div>';
        echo '</div></div>';*/
        
        echo '
		<div class="tab-page" id="step8" '.($obj->id != null ? "" : "style='height: 180px;'").'>
				<h4 class="tab">4. '.$this->l('Accesories').'</h4>';
        if($obj->id != null)
        {
        $accessories = Product::getAccessoriesLight(intval($cookie->id_lang), $obj->id);
        ...
        echo '
					<tr id="extra33" style="display:none">
						<td class="col-left">'.$this->l('Accessories:').'<br /><br /></td>
						<td style="padding-bottom:5px;">
							<div id="divAccessories" id_product="'.$obj->id.'">';
                            
					foreach ($accessories as $accessory){
                        echo '<div>';
						echo $accessory['name'].(!empty($accessory['reference']) ? ' ('.$accessory['reference'].')' : '').' <span onclick="eliminarAccesorio('.$obj->id.', '.$accessory['id_product'].',this);" style="cursor: pointer;"><img src="../img/admin/delete.gif" class="middle" alt="" /></span><br />';
                        echo '</div>';
                    }
					echo '</div>
							<input type="hidden" name="inputAccessories" id="inputAccessories" value="';
