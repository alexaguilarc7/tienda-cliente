...
class AdminAddresses extends AdminTab
{
	...
	public function __construct()
	{
		...
		'id_customer' => array('title' => $this->l('ID Cliente'), 'align' => 'center', 'width' => 30),
	...
	public function displayForm($isMainTab = true)
	{
        ...
        $customerInf = new Customer($obj->id_customer);
        $customerStats = $customerInf->getStats();
        if($obj->id)
            echo '
    		<br />
    		<fieldset style="width: 400px; background: #bfdfe4;">
    			<legend><img src="../img/admin/tab-customers.gif" /> '.$this->l('Customer information').'</legend>
    			<span style="font-weight: bold; font-size: 14px;"><a href="?tab=AdminCustomers&id_customer='.$customerInf->id.'&viewcustomer&token='.Tools::getAdminToken('AdminCustomers'.intval(Tab::getIdFromClassName('AdminCustomers')).intval($cookie->id_employee)).'"> '.$customerInf->firstname.' '.$customerInf->lastname.'</a></span> ('.$this->l('#').$customerInf->id.')<br />
    			(<a href="mailto:'.$customerInf->email.'">'.$customerInf->email.'</a>)<br />
                '.$this->l('Newsletter:').' '.($customerInf->newsletter ? '<img src="../img/admin/enabled.gif" />' : '<img src="../img/admin/disabled.gif" />').'<br />
    			<!--'.$this->l('Account registered:').' '.Tools::displayDate($customerInf->date_add, intval($cookie->id_lang), true).'<br />-->
		...
		switch ($this->addressType)
		{
			...
			default:
				...
				<div class="margin-form">
					<input type="text" size="33" name="alias" id="alias" value="'.htmlentities($this->getFieldValue($obj, 'alias'), ENT_COMPAT, 'UTF-8').'" /> <sup>*</sup>
		...
		if ($this->addressType != 'manufacturer')
		{
				...
				<div class="margin-form">
					<input type="text" size="33" name="address1" id="address1" value="'.htmlentities($this->getFieldValue($obj, 'address1'), ENT_COMPAT, 'UTF-8').'" /> <sup>*</sup>
				</div>
