...
class		Product extends ObjectModel
{
	...
	protected $fieldsValidate = array(
        ...
        'quantity_missing' => 'isUnsignedInt',
		'price' => 'isPrice',
	...
	public function getFields()
	{
        ...
        $fields['quantity_missing'] = intval($this->quantity_missing);
...
public static function getIdByReference($reference)
	...
	*/	
	public static function existsInDatabase($id_product)
	{
		...
		public static function existsInDatabaseByName($reference)
	{
		$row = Db::getInstance()->getRow('
		SELECT `id_product`
		FROM '._DB_PREFIX_.'product p
		...
		if( isset($row['id_product'])) return $row['id_product']; else return 0;
	}
    ...
    static public function updateProductLang($id_product, $id_lang, $field)
    {   
        Db::getInstance()->AutoExecute(_DB_PREFIX_.'product_lang', $field, 'UPDATE', '`id_product` = '.intval($id_product).' AND `id_lang` = '.intval($id_lang));
    }
    ...
    static public function setChangeMetas($id_product)
    {
        $productLangs = Product::getProductsLang($id_product);
        $product = ExportReports::getProduct($id_product);
        
        foreach($productLangs as $productLang):
            if($productLang["name"] != ""):
                if($productLang["meta_title"] == "")
                    $data["meta_title"] = $product["reference"]." ".strtolower($productLang["name"]);
                if($productLang["meta_keywords"] == "")
                    $data["meta_keywords"] = $product["reference"]." ".strtolower($productLang["name"]);
                if($productLang["meta_description"] == "")
                    $data["meta_description"] = strtolower($productLang["description_short"]);   
                Product::updateProductLang($id_product, $productLang["id_lang"], $data);
            endif;
        endforeach;
    }
    
    static public function quantityAddUpdate($id_product, $type, $aux_quantity)
    {
        ...
        /*
            cuando ingrese en regularizar positivo una cantidad x, tiene que restar con el stock negativo y sumar al stock positivo
            cuando ingrese en regularizar negativo una cantidad x, tiene que restar con el stock positivo y sumar al stock negativo
        */
        
        if($type == 'add_stock'){
            $res_positivo = $aux_quantity - $quantity_missing;
            
            if($res_positivo < 0)
            {
                $res_positivo = $res_positivo*(-1);
                $res = $quantity + 0;
                $res2 = $res_positivo;
                $result = Product::updateProductQuantity($product->id, $res);
                $result2 = Product::updateProductQuantityMissing($product->id, $res2);
            }elseif($res_positivo > 0){
                $res = $quantity + $res_positivo;
                $res2 = 0;
                $result = Product::updateProductQuantity($product->id, $res);
                $result2 = Product::updateProductQuantityMissing($product->id, $res);
            }
            $aux_res = 'stock';
        }elseif($type == 'add_stock_missing'){
            $res_negativo = $aux_quantity - $quantity;
            
            if($res_negativo < 0)
            {
                $res = $res_negativo*(-1);
                $result = Product::updateProductQuantity($product->id, $res);
                $res2 = $quantity_missing + 0;
                $result2 = Product::updateProductQuantityMissing($product->id, $res2);
            }elseif($res_negativo > 0){
                $res = 0;
                $res2 = $quantity_missing + $res_negativo;
                $result = Product::updateProductQuantity($product->id, $res);
                $result2 = Product::updateProductQuantityMissing($product->id, $res2);
            }
            $aux_res = 'stock_missing';
        }elseif($type == 'update_stock'){
            $res = $aux_quantity;
            $result = Product::updateProductQuantity($product->id, $res);
            $res2 = $quantity_missing;
            $aux_res = 'stock';
        }elseif($type == 'update_stock_missing'){
            $res = $quantity;
            $res2 = $aux_quantity;
            $result = Product::updateProductQuantityMissing($product->id, $res2);
            $aux_res = 'stock_missing';
        }
        //$datas["type"] = $aux_res;
        $datas["positivo"] = $res;
    ...
    public static function updateProductQuantity($id_product, $quantity)
    {
        $result = Db::getInstance()->Execute('
    ...
    public static function updateProductQuantityMissing($id_product, $quantity)
    {
        $result = Db::getInstance()->Execute('
                UPDATE `'._DB_PREFIX_.'product`
                SET `quantity_missing` = '.$quantity.'
                WHERE `id_product` = '.intval($id_product));
    }
    
    static public function validColumnsProduct($datas, $type)
    {
        $res = true;
        $array_columns = Category::validCharacterDatas($type);
        
        foreach($datas as $value)
        {
            $in_array = in_array($value, $array_columns);
            if($in_array == false){
                return array("message" => "error_column", "column" => $value, "valid" => false);
            }
        }
        return array("message" => "success_column", "column" => "", "valid" => $res);
    }
    
    static public function checkExcelProduct($ordenado)
    {
        ...
        {
            foreach($ordenado[$linea] as $key => $value)
            {
                $cantChart = strlen($value);
                ...
                if($value != "")
                {
                    ...
                    {
                        if($cantChart > 32)
                            return array("message" => "error", "column" => $key);  
                    }elseif($key == "Name-lang-1"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);
                    }elseif($key == "Name-lang-2"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Name-lang-3"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Name-lang-4"){
                        if($cantChart > 128)
                            return array("",$key);    
                    }elseif($key == "Name-lang-5"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Active"){
                        if($value < 0 || $value > 1)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Price-without-TAX"){
                        $decimal_array = explode(",", $value);
                        $dec1 = strlen($decimal_array[0]);
                        $dec2 = strlen($decimal_array[1]);
                        if($dec1 > 10 || $dec2 > 6)
                            return array("message" => "error", "column" => $key);
                    }elseif($key == "TAX"){
                        $decimal_array = explode(",", $value);
                        ...
                        if($dec1 > 10 || $dec2 > 6)
                            ...
                            if($value < 1 || $value > 100)
                                return array("message" => "error", "column" => $key);    
                    }elseif($key == "Sale-from"){
                        $validDate = Product::datecheck($value, "ymd");
                        if(!$validDate)
                    		return array("message" => "error", "column" => $key);    
                    }elseif($key == "Sale-up"){
                        $validDate = Product::datecheck($value, "ymd");
                        ...
                        if($stock  > 10)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-available-lang-1"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-available-lang-2"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-available-lang-3"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-available-lang-4"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-available-lang-5"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-off-stock-lang-1"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-off-stock-lang-2"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-off-stock-lang-3"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-off-stock-lang-4"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Text-off-stock-lang-5"){
                        if($cantChart > 255)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Supplier"){
                        if($cantChart > 64)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Ref-supplier"){
                        if($cantChart > 32)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Description-small-lang-1"){
                        if($cantChart > 400)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Description-small-lang-2"){
                        if($cantChart > 400)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Description-small-lang-3"){
                        if($cantChart > 400)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Description-small-lang-4"){
                        if($cantChart > 400)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Description-small-lang-5"){
                        if($cantChart > 400)
                            return array("message" => "error", "column" => $key);   
                    }else{
                        $aux_message = "success";
        ...
        return array("message" => $aux_message);
    }
    
    static public function datecheck($input,$format="")
    {
    ...
    static public function getLanguageProducts()
    {
        ...
        foreach($products as $product):
            ...
            $aux_array = array_merge(array($position), $products_lang[0], $categories, array($reference));
    ...
    static public function getCategoryProducts($id_category)
    {
        $query = Db::getInstance()->ExecuteS('
            SELECT *
            FROM `'._DB_PREFIX_.'product`
